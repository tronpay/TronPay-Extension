...
export default class Background {
  ...
  static setSeed (sendResponse, _seed) {
    ...
    sendResponse(true)
    // send content
    TabsMessage.widthPayload(TabsMessageTypes.LOCK_STAUS, {unlocked: this._isUnlocked()}).send()
  }
  ...
  static getAddress (sendResponse) {
    let address = this._isUnlocked() ? this._getLocalData().currentAccount.address : ''
  ...
  static setCurrentAccount (sendResponse, message) {
    TabsMessage.widthPayload(TabsMessageTypes.ADDRESS_CHANGED, {address: message.payload.address}).send()
    sendResponse(true)
  }

  static setCurrentNetwork (sendResponse, message) {
    TabsMessage.widthPayload(TabsMessageTypes.NETWORK_CHANGED, {network: message.payload.network}).send()
    sendResponse(true)
  }
  ...
  static encryptKeystore (sendResponse, message) {
    if (this._isUnlocked()) {
      ...
      if (utils.validatePrivateKey(privateKey)) {
        ...
        sendResponse({keystore: keystore})
      } else {
        sendResponse(false)
      }
    } else {
