<template>
  <div class="action" @click="goTxDetail">
    ...
    <div class="action-main">
      ...
      <div class="create-time">{{(data.raw_data.timestamp || data.raw_data.expiration) | moment('YYYY.MM.DD HH:mm:ss')}}</div>
    </div>
    <div class="action-right">
      <span class="action-eos">
        <span v-if="money !== ''">{{money}} TRX</span>
      </span>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    name () {
      ...
      if (this.type === 'TransferContract' || this.type === 'TransferAssetContract') {
    ...
    money () {
      ...
      switch (this.type) {
        case 'TransferContract':
          amount = utils.getTokenAmount(this.data.raw_data.contract[0].parameter.value.amount)
          break
        case 'TransferAssetContract':
          amount = this.data.raw_data.contract[0].parameter.value.amount
          break
        case 'FreezeBalanceContract':
          amount = utils.getTokenAmount(this.data.raw_data.contract[0].parameter.value.frozen_balance)
          break
        case 'UnfreezeBalanceContract':
          amount = 0
          break
        case 'TriggerSmartContract':
          amount = utils.getTokenAmount(this.data.raw_data.contract[0].parameter.value.call_value) | 0
          break
        default:
          amount = 0
      }
    ...
    icon () {
      ...
      switch (this.type) {
        case 'TransferContract':
        case 'TransferAssetContract':
          img = this.currentAccount.address === this.data.raw_data.contract[0].parameter.value.to_address
