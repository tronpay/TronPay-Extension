  ...
  <section>
    ...
    <section class="app-container">
      ...
      <section class="show-vote">
        ...
        <section class="vote">
          ...
          <span class="value">{{tronPower}} TP</span>
        </section>
        <section class="pledged">
          {{$t('label.donePledge')}}：{{tronPower}}
        </section>
        <section class="redeem-time" v-if="tronPower > 0">
          {{$t('label.redeemTime')}}：{{frozenExpires | moment('YYYY.MM.DD HH:mm')}}
        </section>
      </section>
      <section class="btn-group">
        <el-button type="primary" @click="pledgeVisible = true">{{$t('button.pledge')}}</el-button>
        <el-button type="primary" :disabled="!(tronPower > 0)" @click="redeemVisible = true">{{$t('button.redeem')}}</el-button>
      </section>
    </section>
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        <el-button class="full-btn" type="primary" @click="pledgeHandler('pledge')">{{$t('button.pledgeSure')}}</el-button>
      </span>
    </el-dialog>
    ...
    <el-dialog
      top="15vh"
      center
      :title="$t('title.redeemTrx')"
      :visible.sync="redeemVisible">
      <el-form ref="redeem">
        <el-form-item :label="$t('label.redeemAmount')">
          <el-input class="no-border" v-model="tronPower" type="text" :disabled="true">
            <template slot="append">TRX</template>
          </el-input>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button class="full-btn" type="primary" @click="redeemHandler">{{$t('button.redeemSure')}}</el-button>
      </span>
    </el-dialog>
    <!-- alert -->
