  ...
  <section class="app-container">
    <section class="select-lang no-bg">
      <el-select class="language-select" v-model="lang" @change="changeLanguage">
        <el-option v-for="(item, index) in langs" :key="index" :label="item.name" :value="item.value"></el-option>
      </el-select>
    </section>
    <section class="logo">
      <img class="block-center" src="/images/index-logo.png" alt="">
    </section>
    <h2 class="text-center index-title">TronPay</h2>
    <el-form ref="form" :model="formData" :rules="formRules" class="mt20">
      ...
      <el-form-item prop="repassword">
        <el-input class="no-border" v-model="formData.repassword" type="password" :placeholder="$t('placeholder.repassword')"></el-input>
      </el-form-item>
      <el-form-item class="mt80">
        <el-button class="full-btn" type="primary" @click="createWallet('form')">{{$t('button.create')}}</el-button>
      </el-form-item>
    </el-form>
  ...
  methods: {
    ...
    createWallet (formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          this.$router.push({ name: 'agreement', params: { password: this.formData.password } })
        }
      })
    },
    changeLanguage () {
